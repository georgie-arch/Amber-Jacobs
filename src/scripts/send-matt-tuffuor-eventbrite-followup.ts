...
Just wanted to follow up and see if you would still be open to a call.

We have put together a short page outlining an idea for what we could do with Eventbrite next year:
https://powerhouse.indvstryclvb.com/eventbrite

It should give you a clear sense of the direction. We can talk through the thinking in more detail when we speak.

If useful, you can book a time directly into George's calendar here:
https://calendly.com/itsvisionnaire/30min

Or just send over a time that suits and we will work around you.
...
async function main() {
  ...
  if (messages && messages.length > 0) {
    ...
    const message: any = {
      subject: 'Quick follow-up re Eventbrite',
      body: { contentType: 'HTML', content: buildHtml(replyText) },
