...
const replyText = `Hi Didier,

Friday at 12pm works perfectly - let's do it.

Really looking forward to the conversation. Speak then.
...
async function main() {
  ...
  const search = await axios.get(
    `https://graph.microsoft.com/v1.0/me/messages?$search="from:didier@pphe.com"&$top=1&$select=id,subject`,
  ...
  if (messages && messages.length > 0) {
    ...
    console.log(`Found: "${messages[0].subject}" — replying in thread`);
    ...
    console.log('Reply sent to Didier in-thread.');
  } else {
    console.log('Thread not found — sending fresh.');
    const message: any = {
      subject: 'Re: Pre-Cannes Mixer at art\'otel',
      body: { contentType: 'HTML', content: buildHtml(replyText) },
    ...
    console.log('Fresh email sent to Didier.');
  }
}
