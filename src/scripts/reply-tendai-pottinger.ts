...
async function main() {
  ...
  // Find the most recent email from Tendai
  const search = await axios.get(
    `https://graph.microsoft.com/v1.0/me/messages?$search="from:tendai.pottinger@hotmail.com"&$top=1&$select=id,subject,conversationId`,
  ...
  if (messages && messages.length > 0) {
    ...
    const replyBody: any = {
      message: {
        body: { contentType: 'HTML', content: buildHtml('No worries at all, Tendai.\n\nGeorge') },
    ...
    console.log('Reply sent to Tendai Pottinger in-thread.');
  } else {
    // No thread found — send fresh
    console.log('No existing thread found — sending fresh email.');
    const logoB64 = getLogoBase64();
    const message: any = {
      subject: 'Re: Indvstry Power House',
      body: { contentType: 'HTML', content: buildHtml('No worries at all, Tendai.\n\nGeorge') },
      toRecipients: [{ emailAddress: { address: 'tendai.pottinger@hotmail.com', name: 'Tendai Pottinger' } }],
      from: { emailAddress: { address: process.env.EMAIL_USER || '', name: 'George Guise' } },
    };
    if (logoB64) {
      message.attachments = [{
    ...
    console.log('Sent fresh email to Tendai Pottinger.');
  }
}
